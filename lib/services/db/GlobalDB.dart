...
class MediaPlaylistDB {
  ...
  MediaPlaylistDB({required this.playlistName, this.lastUpdated});
...
class AppSettingsBoolDB {
  ...
  String settingName;
  bool settingValue;
  AppSettingsBoolDB({required this.settingName, required this.settingValue});
  ...
  Map<String, dynamic> toMap() {
    return {'settingName': settingName, 'settingValue': settingValue};
  }
...
class RecentlyPlayedDB {
  Id? id;
  DateTime lastPlayed;
  RecentlyPlayedDB({this.id, required this.lastPlayed});
  IsarLink<MediaItemDB> mediaItem = IsarLink<MediaItemDB>();

  Map<String, dynamic> toMap() {
    return {'id': id, 'lastPlayed': lastPlayed.millisecondsSinceEpoch};
  }
...
class NotificationDB {
  ...
  String? payload;
  DateTime? time;
  // This class is kept as a placeholder as per instructions.
  // Logic migrated to NotificationModel.
}
...
class SearchHistoryDB {
  ...
  SearchHistoryDB({required this.query, required this.lastSearched});
...
class PlayStatisticsDB {
  ...
  int playCount;
  DateTime lastPlayed;
  // Store list of timestamps for granular time-period filtering
  List<DateTime> playTimestamps;
...
class SavedQueueDB {
  Id? id = Isar.autoIncrement;
  @Index()
  String queueName; // "current" or timestamp
  DateTime savedAt;

  // Storing media items as JSON strings to avoid complex linking for simple persistence
  List<String> mediaItemsJson;
  int currentIndex;
  int positionMs;
