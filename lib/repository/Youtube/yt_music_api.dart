...
class YtMusicService {
  ...
  Future<List<Map>> getMoodsAndGenres() async {
    if (headers == null) {
      await init();
    }
    await initLanguage();
    ...
    try {
      final body = Map.from(context!);
      body['browseId'] = 'FEmusic_moods_and_genres';
      final Map response =
      ...
      // Navigation path typically involves sectionListRenderer -> ... -> musicNavigationButtonRenderer
      ...
      if (sections is List) {
        for (var section in sections) {
          final gridItems = nav(section, ['gridRenderer', 'items']);
          if (gridItems is List) {
            for (var item in gridItems) {
              final button = nav(item, ['musicNavigationButtonRenderer']);
              if (button != null) {
                final title = nav(button, ['buttonText', 'runs', 0, 'text']);
                final color = nav(button, ['solid', 'leftStripeColor']);
                final browseId =
                    nav(button, ['clickCommand', 'browseEndpoint', 'browseId']);
                final params =
                    nav(button, ['clickCommand', 'browseEndpoint', 'params']);
      ...
      return List<Map>.from(items);
    } catch (e) {
      dev.log('Error fetching moods and genres: $e', name: "YTM");
      return [];
    }
  }
  ...
  Future<void> clearVisitorData() async {
    if (headers != null) {
      ...
      // Re-initialize to get fresh visitor data on next request if needed?
      // Or just leave it empty to force new generation.
      // Actually we might want to nuke it so next call calls 'init' again implies headers=null?
      headers = null;
      dev.log('Visitor data cleared', name: 'YTM');
    }
  }
